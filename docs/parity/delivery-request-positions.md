# delivery-request-positions Parity Map (100% required)

## Scope
- **Legacy:** JSP `src/main/webapp/jsp/DeliveryRequestPositions.jsp`, Struts mapping(s): `<form-bean name="DeliveryRequestPositions" type="net.sam.dcl.form.DeliveryRequestPositionsForm"/>`; `<forward name="selectDocsForDeliveryRequest" path="/DeliveryRequestPositionsAction.do?dispatch=input"/>`; `<action path="/DeliveryRequestPositionsAction" type="net.sam.dcl.action.DeliveryRequestPositionsAction"`; `<forward name="form" path=".DeliveryRequestPositions"/>`.
- **Modern:** route `/delivery-request-positions` (controller: `DeliveryRequestPositionsController` / `DeliveryRequestPositionsController.java`), template ``modern/src/main/resources/templates/delivery-request-positions.html``, service `DeliveryRequestPositionsService.java`, DTO `DeliveryRequestPositionsDto.java`.
...
| `needReload` | `id` (approx) | string/flag (by control type) | Legacy: UI/dispatch-driven; Modern: no explicit Bean Validation in scaffold | editable (unless role/grid checker) | Derived from JSP controls and modern template fields. |
...
| `manager.userFullName` | `id` (approx) | string/flag (by control type) | Legacy: UI/dispatch-driven; Modern: no explicit Bean Validation in scaffold | editable (unless role/grid checker) | Derived from JSP controls and modern template fields. |
...
|---|---|---|---|
| `dispatch=clear` | `GET/POST /delivery-request-positions` (method-specific analogue) | form-bound params | Legacy dispatch action; Modern controller returns same template with model update. |
| `dispatch=filter` | `GET/POST /delivery-request-positions` (method-specific analogue) | form-bound params | Legacy dispatch action; Modern controller returns same template with model update. |
| `dispatch=add` | `GET/POST /delivery-request-positions` (method-specific analogue) | form-bound params | Legacy dispatch action; Modern controller returns same template with model update. |
| `dispatch=delete` | `GET/POST /delivery-request-positions` (method-specific analogue) | form-bound params | Legacy dispatch action; Modern controller returns same template with model update. |
| `dispatch=save` | `GET/POST /delivery-request-positions` (method-specific analogue) | form-bound params | Legacy dispatch action; Modern controller returns same template with model update. |
