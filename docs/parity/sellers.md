# sellers Parity Map (100% required)

## Scope
- **Legacy:** JSP `src/main/webapp/jsp/Sellers.jsp`, Struts mapping(s): `<form-bean name="Sellers" type="net.sam.dcl.form.SellersForm"/>`; `<form-bean name="SellersList" type="net.sam.dcl.form.SellersListForm"/>`; `<action path="/SellersAction" type="net.sam.dcl.action.SellersAction" input=".Sellers" name="Sellers"/>`; `<forward name="back" path="/SellersAction.do?dispatch=execute"/>`.
- **Modern:** route `/sellers` (controller: `SellersController` / `SellersController.java`), template ``modern/src/main/resources/templates/sellers.html``, service `SellersService.java`, DTO `SellersDto.java`.
...
|---|---|---|---|
| `input/show/process` (legacy pattern) | `GET /sellers`, `GET /sellers/{id}`, `POST /sellers` | screen DTO/form params | Legacy Struts dispatch vs Modern MVC handlers. |
