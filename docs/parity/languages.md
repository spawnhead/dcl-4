# languages Parity Map (100% required)

## Scope
- **Legacy:** JSP `src/main/webapp/jsp/Languages.jsp`, Struts mapping(s): `<form-bean name="Languages" type="net.sam.dcl.form.LanguagesForm"/>`; `<action path="/LanguagesAction" type="net.sam.dcl.action.LanguagesAction" input=".Languages"`; `<forward name="back" path="/LanguagesAction.do"/>`; `<action path="/LanguagesListAction" type="net.sam.dcl.action.LanguagesListAction" input=".default-map"/>`.
- **Modern:** route `/languages` (controller: `LanguagesController` / `LanguagesController.java`), template ``modern/src/main/resources/templates/languages.html``, service `LanguagesService.java`, DTO `LanguagesDto.java`.
...
|---|---|---|---|
| `input/show/process` (legacy pattern) | `GET /languages`, `GET /languages/{id}`, `POST /languages` | screen DTO/form params | Legacy Struts dispatch vs Modern MVC handlers. |
