# purposes Parity Map (100% required)

## Scope
- **Legacy:** JSP `src/main/webapp/jsp/Purposes.jsp`, Struts mapping(s): `<form-bean name="Purposes" type="net.sam.dcl.form.PurposesForm"/>`; `<form-bean name="PurchasePurposes" type="net.sam.dcl.form.PurchasePurposesForm"/>`; `<form-bean name="PurposesList" type="net.sam.dcl.form.PurposesListForm"/>`; `<form-bean name="PurchasePurposesList" type="net.sam.dcl.form.PurchasePurposesListForm"/>`.
- **Modern:** route `/purposes` (controller: `PurposesController` / `PurposesController.java`), template ``modern/src/main/resources/templates/purposes.html``, service `PurposesService.java`, DTO `PurposesDto.java`.
...
|---|---|---|---|
| `input/show/process` (legacy pattern) | `GET /purposes`, `GET /purposes/{id}`, `POST /purposes` | screen DTO/form params | Legacy Struts dispatch vs Modern MVC handlers. |
